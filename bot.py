...
# ==================== SETSTORAGE ====================
@bot.on_message(filters.command("setstorage") & filters.private)
async def setstorage_cmd(client, message):
    uid = message.from_user.id
    if is_banned(uid): return
    pending_storage_set[uid] = True
    await message.reply(
...
async def cb_upload(client, cq):
    await cq.answer()
    await cq.message.reply(
        "📤 **File Upload Kaise Kare:**\n\n"
        "1. Pehle /setstorage karo\n"
        "2. Seedha file/photo/video bhejo\n"
        "3. Password do ya /skip karo\n"
        "4. Link mil jayega! 🔗"
    )
...
async def help_cmd(client, message):
    ...
    if uid == OWNER_ID:
        return await message.reply(
            ...
            "**File Commands:**\n"
            "📤 File seedha bhejo → link milega\n"
            "📁 `/myfiles` — files dekho\n"
            "🗄 `/setstorage` — storage set karo\n"
