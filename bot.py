...
# ---------------- Database (TinyDB - No account needed!) ----------------
db = TinyDB("bot_database.json")
users_col = db.table("users")
files_col = db.table("files")
daily_col = db.table("daily_stats")
...
def bump_daily(field):
    key = today_key()
    doc = daily_col.get(Daily._id == key)
    if doc:
        daily_col.update({field: doc.get(field, 0) + 1}, Daily._id == key)
    else:
        daily_col.insert({"_id": key, field: 1})


def get_today_stats():
    doc = daily_col.get(Daily._id == today_key())
    return {
...
def add_user(user_id):
    if not users_col.get(User._id == user_id):
        users_col.insert(
            {"_id": user_id, "banned": False, "joined_at": datetime.utcnow().isoformat()}
        )
        bump_daily("new_users")


def is_banned(user_id):
    user = users_col.get(User._id == user_id)
    return bool(user and user.get("banned", False))


def ban_user(user_id):
    if users_col.get(User._id == user_id):
        users_col.update({"banned": True}, User._id == user_id)
    else:
        users_col.insert({"_id": user_id, "banned": True, "joined_at": datetime.utcnow().isoformat()})


def unban_user(user_id):
    users_col.update({"banned": False}, User._id == user_id)


def get_all_users():
    return [u["_id"] for u in users_col.all()]


def total_users():
    return len(users_col.all())
...
def get_file(file_id):
    return files_col.get(File._id == file_id)


def get_all_files(limit=20):
    all_files = files_col.all()
    all_files.sort(key=lambda x: x.get("created_at", ""), reverse=True)
    return all_files[:limit]


def total_files():
    return len(files_col.all())


def increment_download(file_id):
    doc = files_col.get(File._id == file_id)
    if doc:
        files_col.update({"downloads": doc.get("downloads", 0) + 1}, File._id == file_id)
    bump_daily("downloads")


def set_password(file_id, password):
    return files_col.update({"password": password}, File._id == file_id)


def remove_password(file_id):
    return files_col.update({"password": None}, File._id == file_id)
